...
def make_key():
    ...
    with open('source\\public.pem', 'wb') as key_file:
        key_file.write(pem_pub)
    with open('source\\private.pem', 'wb') as key_file:
        key_file.write(pem_pr)


def take_pr_key():
    with open('source\\private.pem', 'rb') as key_file:
        return rsa.PrivateKey.load_pkcs1(key_file.read(), 'PEM')


def take_pub_key():
    with open('source\\public.pem', 'rb') as key_file:
